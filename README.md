...
=======
## 開発した掲示板のソースコードのテスト（2026/07/23）
* **Repositoryのユニットテスト**
* **目的**データベースへの保存や検索が正しく行えるか確認します
* **目的**データベースへの保存や検索が正しく行えるか確認します
* **使用するもの** @JdbcTest アノテーションを使用すると、データベース操作に必要な最小限の設定で高速にテストを実行できます

* **Controller-Service-Repositoryのインテグレーションテスト**
* **目的**ブラウザからのリクエストがコントローラーに届き、サービス、リポジトリを経て正しくDBに保存されるまでの「一連の流れ」を担保します。
* **目的**データベースへの保存や検索が正しく行えるか確認します
